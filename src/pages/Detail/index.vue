					...
					<div class="choose">
						<div class="chooseArea">
							...
							<dl v-for="(spuSaleAttr, index) in spuSaleAttrList" :key="index">
								...
								<dd
									changepirce="0"
									@click="checkSpuSaleAttrValue(spuSaleAttr.spuSaleAttrValueList, spuSaleAttrValue)"
						...
						<div class="cartWrap">
							<div class="controls">
								<input autocomplete="off" class="itxt" v-model.number="shopNum" @change="shopNumHandler" />
								<a href="javascript:" class="plus" @click="shopNum++">+</a>
								<a href="javascript:" class="mins" @click="shopNum <= 1 ? (shopNum = 1) : shopNum--">-</a>
							</div>
							<div class="add">
...
export default {
	...
	methods: {
		...
		checkSpuSaleAttrValue(spuSaleAttrValueList, spuSaleAttrValue) {
		...
		shopNumHandler() {
			...
			// this.shopNum <= 0 ? (this.shopNum = 1) : "";
		},
		async addShopCart() {
			try {
				await this.$store.dispatch("putGoodsToCart", { skuId: this.skuId, skuNum: this.shopNum });
				//准备一个含有商品名，数量，属性和图片的对象传给购物车页面
				const shopInfo = {
					skuName: this.skuInfo.skuName,
					imgUrl: this.skuInfo.skuDefaultImg,
				...
				this.spuSaleAttrList.forEach(spuSaleAttr => {
					let attr;
					const spuSaleAttrValue = spuSaleAttr.spuSaleAttrValueList.find(spuSaleAttrValue => spuSaleAttrValue.isChecked === "1");
					attr = `${spuSaleAttr.saleAttrName}：${spuSaleAttrValue.saleAttrValueName}`;
					shopInfo.attrs.push(attr);
				});
