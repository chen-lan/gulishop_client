<template>
	<div class="trade-container">
		...
		<div class="content">
			...
			<div class="address clearFix" v-for="(userAddress, index) in userAddressList" :key="userAddress.id" @click="checkAddress(index)">
			...
			<div class="detail">
				...
				<ul class="list clearFix" v-for="goods in detailArrayList" :key="goods.skuId">
					<li>
						<img style="width: 100px" :src="goods.imgUrl" alt="" />
					</li>
					<li>
						<p style="width: 500px">{{ goods.skuName }}</p>
						<h4>7天无理由退货</h4>
					</li>
					<li>
			...
			<ul>
				<li>
					<b
						...
						>件商品，总商品金额</b
					>
					<span>¥{{ tradeInfo.originalTotalAmount }}.00</span>
				</li>
				<li>
					<b>返现：</b>
					<span>0.00</span>
...
export default {
	...
	computed: {
		...
		selectAddress: {
			get() {
				return this.userAddressList.find(userAddress => userAddress.isDefault === "1") || {};
			},
			set(newVal) {
				return newVal;
			},
	...
	methods: {
		...
		checkAddress(index) {
			...
			this.selectAddress = this.userAddressList[index];
		},
		async submitOrder() {
			const submitOrder = {
			...
			try {
				await this.$store.dispatch("submitOrder", submitOrder);
				this.$router.push({
					name: "pay",
					params: {
						orderId: this.$store.state.trade.orderId,
					},
				});
			} catch (error) {
				alert("提交订单失败");
